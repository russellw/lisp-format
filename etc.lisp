...
(defun blankp (a)
  (and (consp a) (eq (car a) +special+) (string= (cadr a) "")))

(defun line-comment-p (a)
  (and (consp a) (eq (car a) +special+) (eql (elt (cadr a) 0) #.(elt ";" 0))))
...
(defun write-text-file (file s)
  (with-open-file (*standard-output* file :direction :output :if-exists :supersede :if-does-not-exist :create)
    (princ s)))
