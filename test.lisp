...
(assert
  (not (subseqp "quick" "the quick brown fox jumped over the lazy dog" 5)))
(assert
  (not (subseqp "quickened" "the quick brown fox jumped over the lazy dog" 4)))
(assert
  (not (subseqp "quick" "the quick brown fox jumped over the lazy dog" 400)))
...
(defun read-string (s)
  (when nil
    (let ((*print-right-margin* 200))
      (format t "~a~99T~a~%"
        s
        (with-input-from-string (*standard-input* s) (lex) (read-tokens)))))
  (with-input-from-string (*standard-input* s) (lex) (read*)))

; Numbers
(assert (equal (read-string "123") 123))
(assert (equal (read-string "+123") 123))
(assert (equal (read-string "-123") -123))
(assert (equal (read-string "123/456") 41/152))
(assert (equal (read-string "-123/456") -41/152))
(assert (equal (read-string "123.5") 123.5))
(assert (equal (read-string "-123.5") -123.5))
(assert (equal (read-string "123.5e6") 1.235e8))
(assert (equal (read-string "-123.5e6") -1.235e8))
...
(assert
  (equal
    ...
    `
      (defun add3 (n)
        (,+special+ "#|(format t \"~&Adding 3 to ~D.\" n)|#")
        (+ n 3))))
(assert
  (equal
    (read-string
...
(defun read-write (s)
  (assert (equal (fmt 0 (read-string s)) s)))
...
(defun write-read (a)
  (assert (equal (read-string (fmt 0 a)) a)))
...
; Comment-case
(assert (equal (fmt 0 (comment-case (read-string ";"))) ";"))
(assert (equal (fmt 0 (comment-case (read-string "; "))) "; "))
(assert (equal (fmt 0 (comment-case (read-string ";foo"))) ";Foo"))
(assert (equal (fmt 0 (comment-case (read-string "; foo"))) "; Foo"))
(assert (equal (fmt 0 (comment-case (read-string ";;foo"))) ";;Foo"))
(assert (equal (fmt 0 (comment-case (read-string ";; foo"))) ";; Foo"))

; Comment-space
(assert (equal (fmt 0 (comment-space (read-string ";"))) ";"))
(assert (equal (fmt 0 (comment-space (read-string "; "))) "; "))
(assert (equal (fmt 0 (comment-space (read-string ";foo"))) "; foo"))
(assert (equal (fmt 0 (comment-space (read-string "; foo"))) "; foo"))
(assert (equal (fmt 0 (comment-space (read-string ";;foo"))) ";; foo"))
(assert (equal (fmt 0 (comment-space (read-string ";; foo"))) ";; foo"))
